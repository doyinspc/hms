...
import { registerInventorytransaction, updateInventorytransaction } from '../../actions/inventorytransaction';
...
const Modals = (props) => {
  ...
  useEffect(() => {
    let params = {
        data:JSON.stringify({'inventoryid':nameid.value}),
        cat:'groupqty',
        table:'inventory_types',
        token:MAIN_TOKEN
      }
  ...
  useEffect(() => {
    ...
    if(parseInt(props.mid) > 0 )
    {
     ...
     populate(props.inventorytransactions.inventorytransaction);  
    }
  ...
  const handleSubmit = (e) =>{
        ...
        fd.append('inventoryid', nameid.value);
        ...
        fd.append('table', 'inventory_transactions');
  ...
  const populate = async(data) =>{
        ...
        let nm1 = {};
        nm1['value'] = data.inventoryid;
        nm1['label'] = data.inventoryname;
        let nm2 = {};
  ...
  const handleInventory = (selected) => {
      setNameid( selected );
  ...
  return (
    <div>
      <Modal isOpen={modal} toggle={toggle} backdrop='static' keyboard={false}>
        <ModalHeader toggle={resetdata}><i className='fa fa-bed'></i> Inventorys Request Form</ModalHeader>
        <ModalBody>
        <Form>
        <FormGroup row>
...
export default connect(mapStateToProps, { registerInventorytransaction, updateInventorytransaction })(Modals)
