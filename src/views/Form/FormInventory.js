...
const Modals = (props) => {
  ...
  useEffect(() => {
    ...
    if(parseInt(props.mid) > 0 )
    {
     ...
     populate(props.inventorytype.inventorytype);  
    }
},[props.mid]);
  ...
  return (
    <div>
      <Modal isOpen={modal} toggle={toggle} backdrop='static' keyboard={false}>
        <ModalHeader toggle={resetdata}><i className='fa fa-bed'></i> Inventories</ModalHeader>
        <ModalBody>
        <Form>
        <FormGroup row>
        <Col sm={12}>
...
const mapStateToProps = (state, ownProps) => ({ 
    ...
    inventorytype:state.inventorytypeReducer.inventorytype
  })
