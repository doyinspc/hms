...
const Modals = (props) => {
  ...
  useEffect(() => {
    let params = {
        data:JSON.stringify({'roomid':nameid.value}),
        cat:'groupqty',
        table:'room_types',
        token:MAIN_TOKEN
      }
  ...
  useEffect(() => {
    ...
    if(parseInt(props.mid) > 0 )
    {
     ...
     populate(props.roomtransactions.roomtransaction);  
    }
    if(props.st1 && parseInt(props.st1) > 0)
    {
      let se = props.roomcategorys.roomcategorys && Array.isArray(props.roomcategorys.roomcategorys) ? props.roomcategorys.roomcategory.filter(rw=>parseInt(rw.id) === parseInt(props.st1))[0]  : [] ;
  ...
  const populate = async(data) =>{
        ...
        let nm1 = {};
        nm1['value'] = data.roomid;
        nm1['label'] = data.roomname;
        let nm2 = {};
  ...
  return (
    <div>
      <Modal isOpen={modal} toggle={toggle} backdrop='static' keyboard={false}>
        <ModalHeader toggle={resetdata}><i className='fa fa-bed'></i> Rooms Request Form</ModalHeader>
        <ModalBody>
        <Form>
        <FormGroup row>
